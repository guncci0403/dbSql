...
WHERE 절 : 조건에 만족하는 행들만 조회되도록 제한 (행을 제한)
                        ex ) sal 컬럼의 값이 1500 보다 큰 사람들만 조회 ==> 7명
...
그중에서 userid 컬람의 값이 'brown'인 행만 조회되도록
WHERE절에 조건을 기술
...
날짜 리터럴 결론 : 문자열 형태로 표현하는 것이 가능하나
                                ...
                                서버 설정과 관계없이 동이하게 해석할 수 있는 방법으로 사용
...
비교대상의 값이 시작값과 종료값 사이에 있을때 참(TRUE)으로 인
(시작값과, 종료값을 포함          비교대상 >= 시작값, 비교대상 <= 종료값)

emp테이블에서 sal 컬럼의 값이 1000 이상 2000 이하인 사원들의 모든 컬럼을 조회
...
where1]
입사일자가 82년1월1일~83년 1월 1일 사이에 있는 사원 조회 (전체 칼럼 조회)
SELECT *
FROM emp
...
IN 연산자
특정 값이 집합 (여래개의 값을 포함)에 포함되어 있는지 여부를 확인
OR연산자로 대체하는 것이 가능
...
emp테이블에서 사원이 10번 부서 혹은 30번 부서에 속한 사원들 정보를 조회(모든 칼럼)
SELECT *
FROM emp
