...
커서를 통해 개발자가 인출하는 과정을 통제함으로 써
테이블 타입변수에 SELECT 결과를 모두 담지 않고도 (메모리 낭비없이)
효율적으로 처리하는 것이 가능하다

커서의 종류
1. 묵시적 커서 - 커서 선언 없이 실행한 SQL에 대해
               오라클 서버가 스스로 생성, 삭제하는 커서
...
emp테이블의 특정 부서에 속하는사원들을 조회할 수 있는 커서를
...
SELECT AVG(diff)
FROM(SELECT dt,         
       LEAD(dt)OVER(ORDER BY dt DESC), dt-LEAD(dt)OVER(ORDER BY dt DESC) diff
FROM dt);
분석함수 없이 작성하기
