...
3. 속성 정의
    ...
    entitiy : 속성들로 이루어진 하나의 데이터(table 1개의 row, 학생 한명, 사원 한명)
    
    식별자 속성 : entity 구분지어줄 수 있는 속성(학생 - 학번, 사원 - 사번)
    일반 속성 : 업무분석을 통해 정의 된 속성
        0. 설계 속성 : 업무에 존재하지 않지만 시스템  설계 과정에서 도출된 속성
                    (코드속성 : 상담 엔터티의 상담코드 :
    ...
    파생 속성 : 다른 속성으로 부터 계산된 값
               ...
               엔터티 : 게시글, 첨부파일
                        ...
                        (SELECT COUNT(*)
                        FROM 첨부파일
                        WHEE 게시글 번호 = 내가원하는 게시글번호;)
                        ==> 쿼리를 매번 이렇게 귀찮아서 게시글 엔터티에 첨부파일 개수라는
                            컬럼을 만들고, 첨부파일의 수가 수정될 때마다 첨부파일, 게시글 엔터티를 동시에 수정
    ...
    2. 대체 여부
      ...
      ** 기준이 모호하다
         ...
         학생 엔터티 : 학생이(xx 학생), 등록일자, 등록을
                    ...
                    where 학번 = '15101001'
4. 관계 정의
    관계 : 두 엔터티 간의 존재하는 업무 규정, 흐름, 논리적인 관계
...
2 정규형 : 주 식별자에 종속이지 않은 속성을 분리
         ...
         (학생, 수강, 강좌 3개의 엔터티가 있고, 수강 엔터티는 학생번호, 강좌번호
          ...
          학생이름은 학생번호 식별자에만 종속이 되고, 강좌번호와는 관련이 없었음
...
집전화번호, 행드폰번호나 동일한 데이터 타입으로 관리하는게 가능할 거 같음
연락처라는 도메인을 생성( VARCHAR2(30) )
학생정보 와 학생이 수강하는 교과목을 관리 하려고 한다.

학생 정보로는 학번 학생이름 생년월일 집주소 핸드폰번호 집전화번호를 관리하고

교과목으로 과목명 수강대상학년 ,교재정보들을 관리하고려고 한다
