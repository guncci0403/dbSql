...
Exception (예외) : pl/sql 블럭이 실행되는 동안 발생하는 에러
                 예외가 발생 했을 때 PL/SQL의 EXCEPTION 절을 기술하여 다른 로직을 실행하여 예외 문제를 해결 할 수 있다.
...
사용자 예외 정의, 사용
1. PL/SQL 블럭안에서 또다른 PL/SQL 블럭을 정의하는 것이 가능
...
CREATE OR REPLACE FUNCTION getDname (p_deptno dept.deptno%TYPE) RETURN VARCHAR2 IS
...
FROM emp;
3. FUNCTION
SELECT empno, ename, deptno, getDname(deptno) dname
FROM emp;

분포도 : 흩어진 정도
데이터 분포도 : 특정 데이터가 발생하는 빈도
예: emp테이블의 empno : 값이 - 14개 존재 - 데이터 분포도가 좋다
   emp테이블의 depno : 값이 - 3개 존재 - 데이터 분포도가 나쁘다

함수의 경우 오라클에서 기본적으로 캐쉬 기능을 사용
기본 캐쉬 사이즈가 20개
getDeptName(10) => ACCOUNTING   
오라클은 10번인자로 getDeptName 실행 했을 때 ACCOUNTING 이라는 결과값을 기억(캐싱)
...
trg_users_pass 트리거에 의해 users_history 테이블에 비밀번호 이력이 생긴것을 볼 수 있다.
SELECT *
FROM users_history;
