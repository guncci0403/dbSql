...
TO_CHAR, TO_DATE 첫번쨰 인자 값을 넣을 때 문자열인지, 날짜인지 구분
...
*****LAST_DAY(DATE) : 주어진 날짜가 속한 월의 마지막 날짜로 반환
LAST_DAY(SYSDATE) : SYSDATE(2020/09/08)가 속한 9월의 마지막 날짜 : 2020/09/30
                    월마다 마지막 일자가 다르기 때문에 해당함수를 통해서 편하게 마지막 이자를 구할 수 있다.
해당월의 가장 첫 날짜를 반환하는 함수는 없어 ==> 모든 월의 첫 날짜 1일 이다
...
오늘 날짜에서 일자를 구하기 ==> 오늘 날짜 - 구한일자 +1
              8           SYSDATE - 8 ==> 2020년 8월 31일 + 1 ==>2020년 9월1일
SELECT TO_DATE(TO_CHAR(SYSDATE, 'YYYYMM') || '01', 'YYYYMMDD'),
       TO_DATE('01', 'DD'),
       TO_DATE(CONCAT(TO_CHAR(SYSDATE, 'YYYYMM'),'01')),
...
두가지 가능한 경우
1.empno(숫자)를 문자로 묵시적 현변환
2.'7369'(문자)를숫자로 묵시적 형변환

알면 매우 좋음, 몰라도 수업 진행하는데 문제 없고, 추후 취업해서도 큰 지장 없음
다만 고급 개발자 와 일반 개발자를 구문하는 차이점이 됨. ==>피피티 참고
...
1.위에서 아래로
2.단 자식노드(들여쓰기가 된 노드)있을경우 자식부터 실행하고 본인 노드를 실행
EXPLAIN PLAN FOR
SELECT *
FROM emp
WHERE empno = '7369';
