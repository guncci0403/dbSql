...
2. 문자 : VARCHAR2(사이즈-byte), CHAR(X)
         ...
         VARCHER2(2000) : 최대 2000BYTE를 담을 수 있는 문자 타입
...
제약조건 : 데이터에 이상한 값이 들어가지 않도록 강제하는 설정
         emp테이블에 empno컬럼에 값이 없는 상태로 들어가는것을 방지
...
1. 테이블을 생성하면서 컬럼 레벨에 제약조건을 생성
        ==> 제약조건을 결합 컬럼(여러개의 컬럼을 합쳐서)에는 적용불가
...
PRIMARY KEY : UNIQUE(중복된 값이 발생할 수 없다) + NOT NULL(NULL값이 들어갈 수 없다)
...
90번 부서가 이미 존재하는 상태 였기 때문에 PRIMARY KEY제약에 의해 정상적으로 입력될 수 없다
unique constraint (GUNCCI.SYS_C007098) violated
INSERT INTO dept_test VALUES ( 90, 'ddit', 'daejeon');

비교
dept 테이블에는 deptno컬럼에 PRIMARY KEY제약이 없는 상태
그렇기 떄문에 deptno 컬럼의 값이 중복이 가능
...
참조 무결성 제약조건에 의해 emp_test 테이블의 deptno컬럼의 값은 dept_test 테이블의 deptno 컬럼에 존재하는 값만 입력이 가능하다
...
90번 부서에 속한 9000번 사원의 deptno컬럼의 값이 NULL로 설정됨
SELECT *
FROM emp_test;
