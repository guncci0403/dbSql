SELECT 컬럼, 익스프레션, 알리아스
empno, enmae, job, hiredate..............8개
...
comm이   NULL인   데이터를    조회  하기  위해  다음과같이   실행할 경우  정상적으로   동작하지    않음
SELECT *
FROM emp
WHERE comm  NULL;
정상 작동
...
WHERE   절에  OR  조건을 사용하게    되면  :   행이  늘어난다
...
mgr가    7698사번을 갖으면선    급여가 1000보다  큰   사원들을    조회
SELECT  *
FROM    emp
WHERE   mgr =   7698
    AND sal >   1000;
    mgr가 7698사번을 이거나 급여가 1000보다  큰   사원들을    조회
SELECT  *
FROM    emp
WHERE   mgr =   7698
    OR sal >   1000;
...
==>오라클은
mgr = 7698 OR mgr = 7839 OR mgr = NULL; 이라고 생각함 IS로 생각하는게 아니라서 NULL값이 안나옴
SELECT *
FROM emp
...
==>오라클은
mgr != 7698 AND mgr != 7839 AND mgr != NULL; 이라고 생각함


where7] job 이SALESMAN, hiredate 1981/06/01이후
data는 대소문자를 가린다
date type 표현
...
내림차순, DESC(디센딩) : 값이 큰 데이터부터 작은 데이터 순으로 나열

ORACLE 에서는 기본적으로 오름차순이 기본 값으로 적용됨
내림차순으로 정렬을 원할 경우 정렬 기준 컬럼 뒤에 DESC를 붙여 준다

job컬럼으로 오름차순 정렬하고, 같은 job를 같는 행끼리는 empno로 내림차순 정렬한다
SELECT *
FROM emp
...
SELECT 절에 *사용 했는데 ,를 통해 다른 특수 컬럼이나 EXPRESSION을 사용 할경우는
      *앞에 해당 데이터가 어떤 테이블에스 왔는지 명시를 해줘야 한다.(한정자)
SELECT ROWNUM, *
FROM emp;
...
별칭은 테이블에도 적용 가능, 단 컬럼이랑 다른 AS 옵션은 없다
