START WITH : 계층쿼리의 시 점(행), 여러개의 행을 조회하는 조건이 들어갈 수도 있다.
...
CONNECT BY PRIOR p_deptcd = deptcd; 상향식
CONNECT BY  p_deptcd = PRIOR deptcd    하향식 데이터를 보고 판단
...
행 제한조건을 WHERE 절에 기술하였을 경우
FROM -> START WITH -> WHERE 절 순으로 실행되기 때문에
1. 계층 탐색을 전부 완료한 후
2. WHERE 절에 해당하는 행만 데이터르 제외한다.
...
WHERE sal = (SELECT MAX(sal)
             FROM emp);
sql의 단점 : 행간 연산이 부족 ==> 해당 부분을 보완 해주는 것이 분석 함수(윈도우 함수)
...
순위 관련 분석함수 - 동일 값에 대한 순위 처리에 따라 3가지 함수를 제공
...
SELECT ename, sal, deptno,
        RANK() over (PARTITION BY deptno ORDER BY sal DESC) sal_rank,
        DENSE_RANK() over (PARTITION BY deptno ORDER BY sal DESC) sal_danse_rank,
        ROW_NUMBER() over (PARTITION BY deptno ORDER BY sal DESC) sal_row_number
FROM emp;

ana1]
SELECT ename, sal, deptno,
        RANK() over (ORDER BY sal DESC, empno desc) sal_rank,
        DENSE_RANK() over (ORDER BY sal DESC, empno desc) sal_danse_rank,
        ROW_NUMBER() over (ORDER BY sal DESC, empno desc) sal_row_number
FROM emp;
