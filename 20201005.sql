...
1. 인위적으로 여러개의 행을 만들기
    ...
    EX : '202010' ==> 31
        ...
        TO_CHAR(날짜, '원하는 항목')
        TO_CHAR(해당 월의 마지막 날짜 구하는 함수(TO_DATE('202010', '포맷')), '원하는항목')
        TO_DATE('202010', 'yyyymm') 날짜 포맷을 정해주지 않았기때문에 1일자 0시 0분 0초 
        ==>마지막 날짜로 변경
...
PL/SQL block 구조 : java try catch와 유사
DECLARE
...
ORACLE 결과 출력을 위해 출력 기능을 활성화 해야함
...
CREATE OR REPLACE PROCEDURE printemp(p_empno IN emp.empno%TYPE) IS
    ...
    v_dname dept.dname%TYPE;
BEGIN
    SELECT emp.empno, emp.ename, dept.dname INTO v_empno, v_ename, v_dname
    FROM emp, dept
    WHERE emp.deptno = dept.deptno
    AND empno = p_empno;
    DBMS_OUTPUT.PUT_LINE('v_empno : ' || v_empno || ', v_ename : ' || v_ename || ', v_dname : ' || v_dname);
END;
/
